...
public class CalculationStatusFilterByTypeAds1248 extends UcqcHelper {
	...
	static String calcStatusFilter = "//span[text()='Filter']";

	/**
	 * Zephyr Test Ticket: ADS-1248 (dev story ADS-757). This script tests that the
	 * correct subset is visible when applying a filter on the Calculation Status
	 * page.
	 */
	@BeforeClass
	public static void setupScript() throws Exception, Throwable {
		ExtentReport.reportCreate("CalculationStatusFilterByTypeAds1248", "webdriver.scripts.status.calculationstatus",
				"CalculationStatusFilterByTypeAds1248");
		try {
	...
	public void test05ConfirmIsNotOperationAndAssertCriteriaMatch() throws Throwable {
		try {
			...
			waitForAjaxExtJs();
			doClick(driver.findElement(By.xpath("//ul/li[text()='Is not']")));
			assertCalcStatusFilterOptionText("condition", "Equal To");
			doSelectCalcStatusFilterCriteria("Value", "Unit Cost Quick Calculation");
			doClick(status.getStatusFilterDialogButtonAdd());
			waitForAjaxExtJs();
	...
	public void assertCalcStatusFilterOptionText(String nameAttribute, String optionText) {
		String filterCriteria = driver.findElement(By.xpath("//input[@name='" + nameAttribute + "']"))
				.getAttribute("value");
	...
	public void assertCalcStatusFilterToMatchCriteriaMessage(String expectedCriteriaMessage) {
		...
		String actualCriteriaMessage = driver
				.findElement(By.xpath(
						"//div[contains(@id,'filtergrid')]/descendant::td[contains(@class,'rowWhiteSpace')]/div"))
				.getText();
		...
		for (WebElement typeValue : typeColumnList) {
			...
			for (int i = 0; i < typeListAsStrings.size(); i++) {
				if (typeListAsStrings.get(i).equals(expectedTypeValue)) {
					System.out.println("The Type Value, " + typeListAsStrings.get(i) + ", is equal to, "
							+ expectedTypeValue + ".");
				} else {
					System.out.println("The Type Value, " + typeListAsStrings.get(i) + ", is not equal to '"
							+ expectedTypeValue + "'.");
					fail();
				}
			}
		...
		for (WebElement typeValue : typeColumnList) {
			...
			for (int i = 0; i < typeListAsStrings.size(); i++) {
				if (typeListAsStrings.get(i).contains(containsText)) {
					System.out.println("The Type Value is, " + typeListAsStrings.get(i) + ", and contains the text, "
							+ containsText + ".");
				} else {
					System.out.println("The Type Value, " + typeListAsStrings.get(i) + ", does not contain the text '"
							+ containsText + "'.");
					fail();
				}
			}
		...
		for (WebElement typeValue : typeColumnList) {
			...
			for (int i = 0; i < typeListAsStrings.size(); i++) {
				if (typeListAsStrings.get(i).contains(startsWithText)) {
					System.out.println("The Type Value is, " + typeListAsStrings.get(i) + ", and starts with the text, "
							+ startsWithText + ".");
				} else {
					System.out.println("The Type Value, " + typeListAsStrings.get(i)
							+ ", does not starts with the text '" + startsWithText + "'.");
					fail();
				}
			}
		}
		...
		for (WebElement typeValue : typeColumnList) {
			...
			for (int i = 0; i < typeListAsStrings.size(); i++) {
				if (typeListAsStrings.get(i).contains(endsWithText)) {
					System.out.println("The Type Value is, " + typeListAsStrings.get(i) + ", and ends with the text, "
							+ endsWithText + ".");
				} else {
		...
		for (WebElement typeValue : typeColumnList) {
			...
			for (int i = 0; i < typeListAsStrings.size(); i++) {
				if (typeListAsStrings.get(i).equals(oneOfText1) || typeListAsStrings.get(i).equals(oneOfText2)) {
					System.out.println("The Type Value is, " + typeListAsStrings.get(i) + ", and is equal to one of, "
							+ oneOfText1 + " or " + oneOfText2 + ".");
				} else {
