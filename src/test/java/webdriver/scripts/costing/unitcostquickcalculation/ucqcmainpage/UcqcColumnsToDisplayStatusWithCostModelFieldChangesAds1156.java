...
/**
 * ADS-1156 (Dev story: ads-999). Last Updated: 06/24/19 Verifies All checkbox
 * and Select button enable-disable properly on Columns to Display section on
 * ucqc page.
 */
...
public class UcqcColumnsToDisplayStatusWithCostModelFieldChangesAds1156 extends UcqcHelper {
	...
	public static void setupScript() throws Throwable {
		ExtentReport.reportCreate("UcqcColumnsToDisplayStatusWithCostModelFieldChangesAds1156",
				"webdriver.scripts.costing.unitcostquickcalculation.ucqcmainpage",
				"UcqcColumnsToDisplayStatusWithCostModelFieldChangesAds1156");
		try {
			costingMap = BuildMap.getInstance(driver, CostingMap.class);
			System.out.println(
					"Test Class: " + UcqcColumnsToDisplayStatusWithCostModelFieldChangesAds1156.class.getSimpleName());
	...
	public void test01VerifyDefaultStatusOfAllCheckboxAndSelectButton() throws Throwable {
		try {
			waitForAjaxExtJs();
			assertTheElementIsDisabled(costingMap.unitCostQuickCalculationButtonColumnsToDisplaySelectBtn(), printout);
	...
	public void test02SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled() throws Throwable {
		try {
			...
			ExtentReport.logPass("PASS",
					"test02SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled");
		} catch (Exception | AssertionError e) {
			ExtentReport.logFail("FAIL",
					"test02SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled", driver, e);
			fail(e.getMessage());
		}
	}
	...
	public void test03UncheckAllCheckboxAndVerifySelectedButtonIsEnabled() throws Throwable {
		try {
			doClick(costingMap.unitCostQuickCalculationAllcheckbx());
			driverDelay();
			// Shilpa on 15.4.2024: commented below lines, when Checkbox is checked need to
			// verify if select button is enabled becoz the checkbox will be enabled
//            assertAllCheckboxIsEnabled(printout);
//            assertColumnsToDisplayAllCheckBoxIsNotChecked();
	...
	public void test05SetCostModelToNoneAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled()
		...
		try {
			...
			assertElementIsDisabled(costingMap.getUnitCostQuickCalculationButtonColumnsToDisplaySelect(), printout);
			ExtentReport.logPass("PASS",
					"test05SetCostModelToNoneAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled");
		} catch (Exception | AssertionError e) {
			ExtentReport.logFail("FAIL",
					"test05SetCostModelToNoneAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled", driver,
					e);
			fail(e.getMessage());
		}
	}
	...
	public void test06SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled() throws Throwable {
		try {
			...
			assertElementIsDisabled(costingMap.getUnitCostQuickCalculationButtonColumnsToDisplaySelect(), printout);
			ExtentReport.logPass("PASS",
					"test06SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled");
		} catch (Exception | AssertionError e) {
			ExtentReport.logFail("FAIL",
					"test06SetCostModelAndVerifyAllCheckboxIsSelectedAndEnabledAndSelectButtonDisabled", driver, e);
			fail(e.getMessage());
		}
	}
	...
	public void test07SetCostModelToAnotherCostModelAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled()
		...
		try {
			doDropdownSelectUsingOptionText(costingMap.getUnitCostQuickCalculationDropdownCostModel(),
					costingMap.getUnitCostQuickCalculationDropdownCostModelMenuList(), newCostModel);
			waitForSpinnerToEnd();
			waitForAjaxExtJs();
			assertColumnsToDisplayAllCheckBoxIsChecked();
			// assertAllCheckboxIsDisabled(printout);
			...
			assertElementIsDisabled(costingMap.getUnitCostQuickCalculationButtonColumnsToDisplaySelect(), printout);
			ExtentReport.logPass("PASS",
					"test07SetCostModelToAnotherCostModelAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled");
		} catch (Exception | AssertionError e) {
			ExtentReport.logFail("FAIL",
					"test07SetCostModelToAnotherCostModelAndEnsureAllCheckboxIsCheckedAndDisabledAndSelectButtonIsDisabled",
					driver, e);
	...
	private void assertAllCheckboxIsEnabled(boolean printout) {
		...
		try {
			waitForAjaxExtJs();
			classText = driver.findElement(By.xpath("//label[text()='All']/ancestor::*[contains(@class,'x-form-cb')]"))
					.getAttribute("class");
		} catch (Throwable e) {
	...
	private void assertColumnsToDisplayAllCheckBoxIsChecked() throws Exception {
		...
		try {
			waitForAjaxExtJs();
			columnsToDisplayCheckBox = driver
					.findElement(By.xpath("//*[contains(@class,'labelValignMiddle')][contains(@id,'checkbox')]"))
					.getAttribute("class");
		} catch (Throwable e) {
			fail("All checkbox not found");
		}
