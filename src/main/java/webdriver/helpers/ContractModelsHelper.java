		...
		try {
			...
			try {
				driver.findElement(By.xpath(
						"//span[text()='Warning']/ancestor::div/descendant::button/span[text()='Cancel & Close']"))
						.click();
				waitForAjaxExtJs();
			} catch (Throwable ee) {
			}
...
//      }
	...
	public List<WebElement> getListOfSections(String testSection) {
		List<WebElement> op = driver.findElements(By.xpath("//*[contains(@id,'customaccordianpanel')][text()='"
				+ testSection + "']/parent::div/following-sibling::div/img"));
		return op;
	}

	public void doChangeMedicareYearTo(String medicareYearRange) throws InterruptedException {
		doDropdownSelectUsingOptionText(editModelMap.getGeneralSectionMedicareYearDropdown(),
				editModelMap.getGeneralSectionMedicareYearDropdownMenu(), medicareYearRange);
	}

	public String calculateOperatingPaymentSectionBlendedRate(double colaOperatingFactor) {
		double natLaborRate = Double
				.parseDouble(driver.findElement(By.name("nationalOperlaborRate")).getAttribute("value"));
		double wageIndex = Double.parseDouble(driver.findElement(By.name("areaWageIndex")).getAttribute("value"));
		double natNonLaborRate = Double
				.parseDouble(driver.findElement(By.name("nationalOperNonLaborRate")).getAttribute("value"));
		double natRelativeWeight = 100; // get from db table: MedicareDRGPricingMechanism2 table

		// Calculation Formula = ( (Nat Labor Rate x Wage Index) + (Nat Non-Lab Rate x
		// COLA Operating Factor) ) x (National Relative Weight / 100)
		Double calculatedBlendedRate = ((natLaborRate * wageIndex) + (natNonLaborRate * colaOperatingFactor))
				* (natRelativeWeight / 100);
	...
	public String calculateOperatingPaymentSectionBlendedRateForPriorYears(double hospitalCapitalRate,
			double hospitalCapitalWeight, double regionalCapitalRate, double regionalCapitalWeight,
			double federalCapitalRate, double nationalCapitalWeight) {

		// Calculation Formula = (Hospital Capital Rate x (Hospital Capital Weight/100))
		// + (Regional Capital Rate x (Regional Capital Rate x (Regional Capital
		// Weight/100)) + (Federal Capital Rate x (National Capital Weight/100))
		Double calculatedBlendedRate = (hospitalCapitalRate * (hospitalCapitalWeight / 100))
				+ (regionalCapitalRate * (regionalCapitalRate * (regionalCapitalWeight / 100)))
				+ (federalCapitalRate * (nationalCapitalWeight / 100));
		DecimalFormat df = new DecimalFormat("#####.##");
		String expectedRate = df.format(calculatedBlendedRate);
		if (printout) {
			System.out.println("Operating Payment section calculated Blended Rate = " + expectedRate);
	...
	public String calculateOperatingPaymentSectionWageAdjustedFederalRate() {
		// Display-only calculated field: (Nat Labor Rate x Wage Index) + Nat Non-Lab
		// Rate
		double natLaborRate = Double
				.parseDouble(driver.findElement(By.name("nationalOperlaborRate")).getAttribute("value"));
		double wageIndex = Double.parseDouble(driver.findElement(By.name("areaWageIndex")).getAttribute("value"));
	...
	public String calculateOperatingPaymentSectionWageAdjustedFederalRateForPriorYears(double nationalCapitalRate,
			double capitalGeographicAdjustmentFactor, double largeUrbanAddOnFactor) {
		// Calculation Formula = nationalCapitalRate x capitalGeographicAdjustmentFactor
		// x largeUrbanAddOnFactor
		Double calculatedBlendedRate = nationalCapitalRate * capitalGeographicAdjustmentFactor * largeUrbanAddOnFactor;
		DecimalFormat df = new DecimalFormat("#####.##");
		String expectedRate = df.format(calculatedBlendedRate);
		if (printout) {
			System.out.println("Operating Payment section calculated Wage Adjusted Federal Rate For Prior Years = "
					+ expectedRate);
		}
		return expectedRate;
	}
	...
	public void doCloseEditSectionAndClickSaveButton(String serviceModel) throws InterruptedException {
		...
		waitForAjaxExtJs();
		driver.findElement(By.xpath(
				"//label[text()='Service Model']/ancestor::div/descendant::div[text() = '" + serviceModel + "'][2]"))
				.click();
		Thread.sleep(2000);
	...
	public String calculateCapitalPaymentBlendedRate(Double colaFactor) {
		Double capGeoAdj = Double
				.parseDouble(getFieldValue(editModelMap.getCapitalPaymentSectionCapitalGeographicAdjFactorField()));
		Double natCapRate = Double
				.parseDouble(getFieldValue(editModelMap.getCapitalPaymentSectionNationalCapitalRateField()));
	...
	public void assertThatDropdownSelectedValue(WebElement dropdownTriggerElement, WebElement dropdownMenu,
	...
	public void verifyServiceIsDisplayedInTextfieldOnMainPage(String availableService1) throws InterruptedException {
		...
		try {
			boolean isDisplayed = driver.findElement(By.xpath(
					"//*[contains(text(),'Service')]/ancestor::div[contains(@class, 'x-grid-header-ct')]/following-sibling::div/descendant::td[contains(@class,'x-grid-cell x-grid-cell-gridcolumn')]/div[contains(text(),'"
							+ availableService1 + "')]"))
					.isDisplayed();
	...
	public int getFilterNumberOfExpectedMatches() throws InterruptedException {
		waitForSpinnerToEnd();
		waitForAjaxExtJs();
		String filterResultsRatio = driver
				.findElement(By.xpath("//*[contains(text(),'Filter to Match These Criteria')]")).getText();
	...
	public String doUpdateFormFieldValueAndGetValueAttribute(WebElement element, String newTestValue,
			boolean printout) {
	...
	public void assertAvailablePaneValueIsGreaterThan(int availablePaneValue) {
		int availableList = Integer.parseInt(driver
				.findElement(By.xpath("//label[contains(text(),'Item(s) Available of')]")).getText().substring(0, 1));
	...
	public void assertInvalidFieldValuesWithClickAwayFromField(WebElement element, WebElement elementToClick,
			String[] invalidExpectedValues) throws InterruptedException {
		for (String value : invalidExpectedValues) {
			...
			assertTrue("This value should be invalid: " + value,
					actualInvalidMessage.contains("The value in this field is invalid"));
		}
	}

	public void assertInvalidFieldValues(WebElement element, String[] invalidExpectedValues)
	...
	public static void displayPricingMethodsSectionForContractModelAndServiceModel(String contractModel,
			String serviceModel) {
		try {
			...
			waitForAjaxExtJs();
			driver.findElement(By.xpath(
					"//td[contains(@class,'x-grid-cell x-grid-cell-gridcolumn')]/*[text()='" + contractModel + "']"))
					.click();
			...
			waitForAjaxExtJs();
			Thread.sleep(2000);
			driver.findElement(By.xpath("//td[contains(@class,'x-grid-cell-treecolumn')]/*[text()='Model Contract']"))
					.click();
			Thread.sleep(5000);
			waitForSpinnerToEnd();
			waitForAjaxExtJs();
			driver.findElement(
					By.xpath("//td[contains(@class,'x-grid-cell-treecolumn')]/*[text()='Define Payment Terms']"))
					.click();
			waitForAjaxExtJs();
			Thread.sleep(5000);
			String feeCheckbox = driver.findElement(By.xpath(
					"//td[contains(@class,'x-grid-cell-treecolumn')]/*[text()='Fee For Service Payment Terms']/input"))
					.getAttribute("class");
			assertTrue(feeCheckbox.contains("checked"));
			driver.findElement(By
					.xpath("//td[contains(@class,'x-grid-cell-treecolumn')]/*[text()='Fee For Service Payment Terms']"))
					.click();
			waitForSpinnerToEnd();
			waitForAjaxExtJs();
			System.out.println(5000);
			driver.findElement(By.xpath(
					"//span[contains(@class,'x-panel-header-text')][text()='Pricing Method']/../following-sibling::div"))
					.click();
			waitForAjaxExtJs();
			System.out.println(1000);
			driver.findElement(By.xpath("//label[text()='Service Model']/ancestor::div/descendant::div[text() = '"
					+ serviceModel + "'][2]")).click();
		} catch (Throwable e) {
			e.getMessage();
		}
	}
...
//		driver.manage().window().maximize();
	...
	public String[] removeAllColumnsToDisplayColumns() throws InterruptedException, Throwable {
		String[] columns = { "Charge Code Name", "Modifier", "Total Unit Cost", "Total Quick Cost", "Total Change",
				"Salaries and Wages RVU", "Quick Salaries and Wages RVU", "Salaries and Wages Cost",
				"Quick Salaries and Wages Cost", "Salaries and Wages Change", "Employee Benefits RVU",
				"Quick Employee Benefits RVU", "Employee Benefits Cost", "Quick Employee Benefits Cost",
				"Employee Benefits Change", "Medical Supplies RVU", "Quick Medical Supplies RVU",
				"Medical Supplies Cost", "Quick Medical Supplies Cost", "Medical Supplies Change",
				"Non-Medical Supplies RVU", "Quick Non-Medical Supplies RVU", "Non-Medical Supplies Cost",
				"Quick Non-Medical Supplies Cost", "Non-Medical Supplies Change", "Equip Repair & Maint RVU",
				"Quick Equip Repair & Maint RVU", "Equip Repair & Maint Cost", "Quick Equip Repair & Maint Cost",
				"Equip Repair & Maint Change", "Direct Depreciation RVU", "Quick Direct Depreciation RVU",
				"Direct Depreciation Cost", "Quick Direct Depreciation Cost", "Direct Depreciation Change",
				"Purchased Services RVU", "Quick Purchased Services RVU", "Purchased Services Cost",
				"Quick Purchased Services Cost", "Purchased Services Change", "Professional Fees RVU",
				"Quick Professional Fees RVU", "Professional Fees Cost", "Quick Professional Fees Cost",
				"Professional Fees Change", "Other Expenses RVU", "Quick Other Expenses RVU", "Other Expenses Cost",
				"Quick Other Expenses Cost", "Other Expenses Change", "Direct Overhead RVU",
				"Quick Direct Overhead RVU", "Direct Overhead Cost", "Quick Direct Overhead Cost",
				"Direct Overhead Change", "Hospital Overhead RVU", "Quick Hospital Overhead RVU",
				"Hospital Overhead Cost", "Quick Hospital Overhead Cost", "Hospital Overhead Change",
				"Corporate Overhead RVU", "Quick Corporate Overhead RVU", "Corporate Overhead Cost",
				"Quick Corporate Overhead Cost", "Corporate Overhead Change", "Depreciation RVU",
				"Quick Depreciation RVU", "Depreciation Cost", "Quick Depreciation Cost", "Depreciation Change",
				"Tech RVU", "Quick Tech RVU", "Tech Cost", "Quick Tech Cost", "Tech Change" };
	...
	public void assertAvailableColumnIsEmpty() {
		String availableColumnsXpath = "//*[contains(@class,'glAccountsGrid')][1]/descendant::tbody";
		// String availableColumnsXpath =
		// "//*[contains(@class,'glAccountsGrid')][1]/descendant::tbody/tr/th";
		WebElement availableMenu = driver.findElement(By.xpath(availableColumnsXpath));
		List<WebElement> actualAvailableColumns = availableMenu.findElements(By.tagName("tr"));
		List<String> actualAvailableColumnNames = new ArrayList<>();
		for (WebElement availableColumns : actualAvailableColumns) {
	...
	public void assertColumnsToDisplayAllCheckBoxIsNotChecked() throws Exception {
		...
		try {
			waitForAjaxExtJs();
			columnsToDisplayCheckBox = driver
					.findElement(By.xpath("//*[contains(@class,'labelValignMiddle')][contains(@id,'checkboxfield')]"))
					.getAttribute("class");
		} catch (Throwable e) {
	...
	public void assertColumnsToDisplayColumnIsSelected(String column) throws Exception {
		...
		try {
			...
			columnIsSelected = driver
					.findElement(By
							.xpath("//*[contains(@class,'x-grid-table')]/descendant::*[text()='" + column + "']/../.."))
					.getAttribute("class");
		} catch (Throwable e) {
			System.out.println("Element Not Found");
			fail("element not found");
	...
	/*
	 * public void selectColumnsToDisplaySelectedColumn(String column) { String
	 * columnPath =
	 * "//*[contains(@class,'glAccountsGrid')][2]/descendant::*[text()='" + column +
	 * "']"; doClick(driver.findElement(By.xpath(columnPath))); } public void
	 * selectColumnsToDisplayAvailableColumn(String column) { String columnPath =
	 * "//*[contains(@class,'glAccountsGrid')][1]/descendant::*[text()='" + column +
	...
	public void assertColumnsToDisplayColumn(String column) {
		...
		// String selectedColumnsXpath =
		// "//*[contains(@class,'glAccountsGrid')][1]/descendant::tbody";
		WebElement selectedMenu = driver.findElement(By.xpath(selectedColumnsXpath));
		List<WebElement> actualSelectedColumns = selectedMenu.findElements(By.tagName("tr"));
		List<String> actualSelectedColumnNames = new ArrayList<>();
...
//	        String columnPath = "//label[text()='Available']/following::*[contains(@class,'glAccountsGrid')]/descendant::*[text()='" + column + "']";
	...
	public static void filterByCalculationStartTimeInCalculationStatusPage() throws Throwable {
		...
		doClick(driver
				.findElement(By.xpath("//div[contains(@id,'statustoolbar')]//span[text()='Filter']/parent::button")));
		doDropdownSelectUsingOptionText(CostingMap.getcalculationFilterPopUpFilterDrop(),
				costingMap.getCalculationFilterDropdownMenuList(), "Import/Export Start Time");
	...
	public static void navigateFeeForServicePaymentTermsScreenSelectionSelectServiceModel(String Model,
			String serviceModel) throws InterruptedException {
		try {
			waitForSpinnerToEnd();
			waitForAjaxExtJs();
			driver.findElement(By.xpath(
					"//span[contains(@class,'x-panel-header-text')][text()='" + Model + "']/../following-sibling::div"))
					.click();
			waitForAjaxExtJs();
			Thread.sleep(2000);
			driver.findElement(By.xpath(
	...
	public static void navigateFeeForServicePaymentTermsScreenSelectionPanel(String Model) throws Throwable {
		try {
			...
			executor.executeScript("arguments[0].click();", driver.findElement(By
					.xpath("//label[text()='Service Model']/ancestor::div/descendant::div[text() = '" + Model + "']")));
			Thread.sleep(300);
		}
	}
	...
	public static void navigateFeeForServicePaymentTermsPageRiskLimiterSectionClickEditButtonToOpenEditDialog() {
		try {
			...
			waitForAjaxExtJs();
			Thread.sleep(1000);
			// assertElementTextWithXpath("//span[contains(@id, 'medicareinpatientpps')]",
			// "Edit Price for " + serviceModel + " [Encounter]", printout);
		} catch (Throwable e) {
			e.getMessage();
		}
	}
...
//			 Thread.sleep(1000);
	...
	public void doFilterCreateForServices(String[] filterParameters) throws Throwable {
		...
		doFilterSetFilterParameterServices(filterParameters[0], filterParameters[1], filterParameters[2],
				filterParameters[3]);
		waitForAjaxExtJs();
	...
	public void doFilterSetFilterParameterServices(String field, String operator, String condition, String value)
			throws InterruptedException {
		waitForAjaxExtJs();
	...
	public static void highlightColumnsToDisplayColumnServices() throws InterruptedException, Throwable {
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",
				ContractingMap.getselectAddServicesButton());
		Thread.sleep(1000);
		doClick(ContractingMap.getselectAddServicesButton());
	}

	public void AssertAddOnPaymentTechnologyServicesDisplayed(WebElement element, String serviceName,
			String[] filterParameters) throws Throwable {
		try {
			doClick(element);
	...
	public void AssertAddOnPaymentTechnologyServicesMaxValue(String serviceName, String value) throws Throwable {
		try {
			...
			System.out.println(driver
					.findElement(By.xpath("(//div[text()='" + serviceName + "']//following::td/div)[1]")).getText());
			if (!((driver.findElement(By.xpath("(//div[text()='" + serviceName + "']//following::td/div)[1]")).getText()
					.equals(value)))) {
				driver.findElement(By.xpath("(//div[text()='" + serviceName + "']//following::td/div)[1]")).click();
				Thread.sleep(500);
				keyInValues(driver.findElement(By.xpath("//div[text()='" + serviceName + "']//following::div")), value);
			}
		} catch (Exception | AssertionError e) {
	...
	public static void enterServicesMaxValue(HashMap<String, String> services) throws Exception {
		...
		for (Map.Entry<String, String> set : services.entrySet()) {
			System.out.println(set.getKey().toString());
			scrollToView("(//div[text()='" + set.getKey().toString() + "']//following::td/div)[1]");
			System.out.println(driver
					.findElement(By.xpath("(//div[text()='" + set.getKey().toString() + "']//following::td/div)[1]"))
					.getText());
			if (!((driver
					.findElement(By.xpath("(//div[text()='" + set.getKey().toString() + "']//following::td/div)[1]"))
					.getText().equals(set.getValue().toString())))) {
				driver.findElement(By.xpath("(//div[text()='" + set.getKey().toString() + "']//following::td/div)[1]"))
						.click();
				Thread.sleep(1500);
				keyInValues(
						driver.findElement(By.xpath(
								"//div[text()='" + set.getKey().toString() + "']//following::input[@name='amount']")),
